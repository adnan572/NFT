...
<div className="flex gap-6 justify-center items-center max-sm:flex-col max-md:w-full flex-wrap">
  ...
  {seller.map((item)=> (
    <TrendingCardAnimation>
        <div className="bg-red-400/20 px-3 py-3 rounded-md bg-black/5 backdrop-blur-sm max-sm:w-full max-md:px-1 w-full lg:w-[90%] xl:w-[80%] mx-auto">
  <div className="flex gap-3 w-full overflow-hidden">
    {item.imgURL.map((pics) => (
      <img
        src={pics}
        alt=""
        className="w-[100px] h-[100px] object-contain object-center rounded-lg max-md:h-auto max-md:max-h-[400px] max-md:w-1/3"
      />
    ))}
  </div>
  ...
  <div className="flex justify-between mb-1">
    ...
    <h1 className="bg-gradient-to-r from-pink-500 via-red-500 to-yellow-500 bg-clip-text text-transparent text-4xl">
      {item.number}
    </h1>
  </div>
  <div className="flex justify-between">
    ...
    <p className="text-slate-300 font-semibold">
      Total earnings: <span className="text-slate-100">{item.total}</span>
    </p>
  </div>
</div>
